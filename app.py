...
import bcrypt,logging,datetime,json,secrets
...
def login():
	if request.method=='POST':
		...
		if "token" in session and session_collection.find_one(_token):
...
def add_u():
	...
	'''_token=request.headers['authorization']
	if "token" in session and session_collection.find_one(_token): # This code is necessary for checking the token in session and i must check the time too that i will add in next update.'''
...
def delete_role():
	...
	if _data["role"] and role_details:
		existing_role=user_collection.find_one({'role':_data['role']})
		if existing_role:
